<%--
    JSP stránka s formulářem představující údaje o uživateli.
    Lze zde přidat a upravit údaje o uživateli.
--%>
...
<div id="content">	
	...
	boolean userDataChecked = false;
        String idUserForm = request.getParameter("saveUser");
        if (idUserForm != null) {
	...
	if (userDataChecked) {
	    ...
	    <h2>Profil uživatele: </h2>
            <form  id="userForm" action="Jidelna" method="post">
                <table>
                    <tr>
                <%
			...
			<td><input type="text" name="email" value="<% out.print(email); %>" <% out.print(readOnly); %>/></td>
                    </tr>
                    <tr>
			...
			<td><input type="text" name="surname" value="<% out.print(surname); %>"/></td>
                    </tr>
                    <tr>
	        ...
	        %>
                    <tr>
		        <td>Administrátor <input type="checkbox" name="admin" <% out.print(checked); %>/></td>
			<td></td>
		    </tr>
		<%  } %>
                    <tr>
                        <td><button class="appButton" type="submit" name="saveUser" value="<% out.print(formBean.getId());%>">Uložit</button></td>
			<td></td>
                    </tr>
                </table>
            </form>
